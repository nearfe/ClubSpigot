...
public class NachoConfig {
	...
	// If the config has been migrated to clubspigot.yml
	...
	public static void init(File configFile) {
		...
		try {
			ClubSpigot.LOGGER.info("Loading NachoSpigot config from " + configFile.getName());
	...
	// Migrates all read data into the ClubSpigot config
	// TODO: save this data to file instead of just putting it in memory
	public static void moveToClubSpigotConfig(Class<?> clazz, Class<?> clubSpigotConfigClazz) {
		for (Field oldField : clazz.getDeclaredFields()) {
			...
			if (Modifier.isPublic(oldField.getModifiers()) && Modifier.isStatic(oldField.getModifiers()) && !oldField.getName().equals("CONFIG_FILE") && !oldField.getName().equals("hasMigrated")) {
				...
				for (Field newField : clubSpigotConfigClazz.getDeclaredFields()) {
				...
				if (!found) {
					ClubSpigot.LOGGER.warn("Unable to migrate option " + oldField.getName() + "!");
				}
			}
 		}
		hasMigrated = true;
