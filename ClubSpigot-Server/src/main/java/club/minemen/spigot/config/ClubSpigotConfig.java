package club.minemen.spigot.config;
...
import club.minemen.spigot.async.pathsearch.AsyncNavigation;
import club.minemen.spigot.entity.EntityTickLimiter;
...
public class ClubSpigotConfig {
	...
	private static final Logger LOGGER = LogManager.getLogger(ClubSpigotConfig.class);
	public static File CONFIG_FILE;
	protected static final YamlCommenter c = new YamlCommenter();
	private static final String HEADER = "This is the main configuration file for ClubSpigot.\n"
			+ "As you can see, there's tons to configure. Some options may impact gameplay, so use\n"
			+ "with caution, and make sure you know what each option does before configuring.\n" + "\n"
			+ "If you need help with the configuration or have any questions related to ClubSpigot,\n"
	...
	public static void init(File configFile) {
		...
		try {
			ClubSpigot.LOGGER.info("Loading ClubSpigot config from " + configFile.getName());
			config.load(CONFIG_FILE);
		} catch (IOException ignored) {
		} catch (InvalidConfigurationException ex) {
			LOGGER.log(Level.ERROR, "Could not load clubspigot.yml, please correct your syntax errors", ex);
		...
		c.setHeader(HEADER);
		readConfig(ClubSpigotConfig.class, null);

		// Move nacho config options to our config
		/*if (!NachoConfig.hasMigrated) {
			// TODO: finish config migration/loading
			//NachoConfig.moveToClubSpigotConfig(NachoConfig.class, ClubSpigotConfig.class);
			LOGGER.info("Successfully loaded nacho.yml into memory!");
			LOGGER.warn("Loading of nacho.yml will be removed in the future, so transfer the config options into clubspigot.yml, then delete nacho.yml");
			makeReadable();
		}*/
		...
		try {
			...
			LOGGER.warn("Please regenerate your clubspigot.yml file to prevent this issue! The server will run with the default config for now.");
	...
	static void loadComments() {
		...
		c.addComment("settings.command", "Configuration for ClubSpigot's commands");
		...
		c.addComment("settings.command.mob-ai", "Enables the command \"/mobai\" which toggles mob ai. Users require the permission clubspigot.command.mobai");
		...
		c.addComment("settings.command.ping.enable", "Enables the command \"/ping <player>\" which shows player ping. Users require the permission clubspigot.command.ping");
		...
		c.addComment("settings.statistics", "Enables ClubSpigot statistics. This allows developers to see how many ClubSpigot servers are running. \nThis has no performance impact and is completely anonymous, but you can opt out of this if you want.");
	...
	private static void serverBrandName() {
		serverBrandName = getString("settings.brand-name", "ClubSpigot");
	}
