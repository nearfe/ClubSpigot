...
public abstract class World implements IBlockAccess {
	...
	public final List<Entity> entityList = new ObjectArrayList<Entity>() { // ClubSpigot - ArrayList -> ObjectArrayList 
		@Override
	...
	public final List<EntityHuman> players = new ObjectArrayList<>(); // ClubSpigot - ArrayList -> ObjectArrayList 
	public final List<Entity> k = new ObjectArrayList<>(); // ClubSpigot - ArrayList -> ObjectArrayList 
	...
	protected boolean lastTickOverload = false; // ClubSpigot
	...
	protected World(IDataManager idatamanager, WorldData worlddata, WorldProvider worldprovider,
		...
		timings = new co.aikar.timings.WorldTimingsHandler(this); // Spigot - code below can generate new world and
																	// access timings
		// ClubSpigot - re-implement Spigot's entity max tick time, but only for certain entities
		this.entityLimiter = new EntityTickLimiter(ClubSpigotConfig.entityMaxTickTime); //new org.spigotmc.TickLimiter(spigotConfig.entityMaxTickTime);
		this.tileLimiter = new org.spigotmc.TickLimiter(ClubSpigotConfig.tileMaxTickTime);
	}
	...
	public boolean addEntity(Entity entity, SpawnReason spawnReason) { // Changed signature, added SpawnReason
		...
		if (!flag && !this.isChunkLoaded(i, j, true)) {
			...
			if (entity instanceof EntityLiving) {
				((EntityLiving) entity).maxNoDamageTicks = ClubSpigotConfig.hitDelay;
			}
			// ClubSpigot end
			
			// ClubSpigot start - configurable potion speeds
			if (entity instanceof EntityPotion) {
				// Subtract the speed from the y value of the velocity, the potion will drop faster and will therefore splash faster
				entity.motY -= ClubSpigotConfig.potionSpeed;	
				
				// Mark velocity as changed only if potion speed is not default
				if (!(ClubSpigotConfig.potionSpeed == 0)) {
...
//        if (!guardEntityList) { // Spigot - It will get removed after the tick if we are ticking // Paper - moved down
	...
	public void tickEntities() {
		boolean tickOverload = false; // ClubSpigot
		...
		entityLimiter.initTick(); // ClubSpigot - re-implement Spigot's entity max tick time, but only for certain entities
		for (tickPosition = 0; tickPosition < entityList.size(); tickPosition++) {
			...
			// ClubSpigot - synchronize
			synchronized (entity) {
				// ClubSpigot start - re-implement Spigot's entity max tick time, but only for certain entities
...
//            this.tileEntityList.removeAll(this.c);//c  == tileEntityListUnload
		...
		lastTickOverload = tickOverload; // ClubSpigot
	}
	...
	public float a(Vec3D vec3d, AxisAlignedBB axisalignedbb) {
		...
		// ClubSpigot start - switch to mathhelper instead of Java math
