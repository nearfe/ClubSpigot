...
public class EntityEnderPearl extends EntityProjectile {
	...
	protected void a(MovingObjectPosition movingobjectposition) {
		...
		if (blockPosition != null) {
			...
			boolean collides = ClubSpigotConfig.pearlPassthroughTripwire && block == Blocks.TRIPWIRE
					|| ClubSpigotConfig.pearlPassthroughCobweb && block == Blocks.WEB
					|| ClubSpigotConfig.pearlPassthroughBed && block == Blocks.BED
					|| ClubSpigotConfig.pearlPassthroughFenceGate
					...
					|| ClubSpigotConfig.pearlPassthroughSlab && (block == Blocks.STONE_SLAB
		...
		if (!this.world.isClientSide) {
			if (entityliving instanceof EntityPlayer) {
				...
				if (entityplayer.playerConnection.a().isConnected() && entityplayer.world == this.world
					...
					if (ClubSpigotConfig.antiEnderPearlGlitch) {
					...
					if (!teleEvent.isCancelled() && !entityplayer.playerConnection.isDisconnected()) {
						...
						ClubSpigot.getInstance().getLagCompensator().registerMovement(player, location); // Nacho
