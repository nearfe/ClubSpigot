...
import club.minemen.spigot.async.entitytracker.MultithreadedEntityTracker;
import club.minemen.spigot.config.ClubSpigotConfig;
import club.minemen.spigot.world.WorldTicker;
...
public class WorldServer extends World implements IAsyncTaskHandler {
	...
	public WorldServer(MinecraftServer minecraftserver, IDataManager idatamanager, WorldData worlddata, int i,
		...
		// ClubSpigot - async entity tracking
		if (ClubSpigotConfig.disableTracking) {
			this.tracker = new EntityTracker(this);
		} else if (ClubSpigotConfig.fullAsyncTracking) {
	...
	public void doTick() {
		...
		if (this.getGameRules().getBoolean("doMobSpawning")
			...
			// ClubSpigot start - disable mob spawning based on tps and server load
			if (!(ClubSpigotConfig.limitedMobSpawns
					&& MinecraftServer.getServer().recentTps[0] < ClubSpigotConfig.limitedMobSpawnsThreshold) // check if tps is low 
					&& !(ClubSpigotConfig.stopMobSpawnsDuringOverload && lastTickOverload)) { // check if server is overloaded
				// ClubSpigot end
