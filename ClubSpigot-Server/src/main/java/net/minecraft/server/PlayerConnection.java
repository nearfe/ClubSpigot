...
import club.minemen.spigot.events.PlayerIllegalBehaviourEvent;
...
public class PlayerConnection implements PacketListenerPlayIn, IUpdatePlayerListBox {
	...
	public void c() {
		...
		if (ClubSpigotConfig.modernKeepalive) {
		...
		if (this.b(packetplayinflying)) {
			...
			if (!this.player.viewingCredits) {
				...
				if ((packetplayinflying.hasPos) && ((delta > 0.0D) && (this.checkMovement && !this.player.dead))) {
					for (MovementListener movementListener : ClubSpigot.getInstance().getMovementListeners()) {
						try {
				...
				if ((delta > 1f / 256 || deltaAngle > 10f) && (this.checkMovement && !this.player.dead)) {
					...
					if (ClubSpigotConfig.firePlayerMoveEvent && PlayerMoveEvent.getHandlerList().getRegisteredListeners().length != 0) { // ClubSpigot - only handle player move event when there are listeners // Spigot - don't skip any move events
						...
						if (event.isCancelled()) {
		                    ClubSpigot.getInstance().getLagCompensator().registerMovement(player, to); // Nacho
                    ...
                    ClubSpigot.getInstance().getLagCompensator().registerMovement(player, to); // Nacho - register movement
				}
	...
	public void a(PacketPlayInBlockPlace packetplayinblockplace) {
		...
		if (packetplayinblockplace.getFace() == 255) {
			...
			if (!throttled) {
				...
				// SportPaper end
			} else if (MinecraftServer.currentTick - lastDropTick > 1 && ClubSpigotConfig.fixEatWhileRunning) {
				this.player.playerInteractManager.useItem(this.player, this.player.world, itemstack);
			}
		...
		if (itemstack == null || itemstack.l() == 0) {
			...
			// KigPaper-0172
			if (ClubSpigotConfig.stopDecodingItemStackOnPlace
	...
	public void sendPacket(final Packet packet) {
		...
		try {
			for (PacketListener packetListener : ClubSpigot.getInstance().getPacketListeners()) {
				try {
	...
	public void a(PacketPlayInUseEntity packetplayinuseentity) {
		...
		if (entity != null) {
			...
			if (!flag) {
                ...
                d0 = (ClubSpigotConfig.improvedHitDetection) ? 12.75D : 9.0D;
            } else {
            	// ClubSpigot start - configurable reach cap            	
            	if (ClubSpigotConfig.creativeBypass && this.player.playerInteractManager.getGameMode() == EnumGamemode.CREATIVE) {
            		d0 = 36.0;
            	} else {
                	d0 = ClubSpigotConfig.maxReachSqrd;
            	}	
            }
...
//        }
		...
		} else if (ClubSpigotConfig.kickOnIllegalBehavior) {
	...
	// ClubSpigot start - queue-able packets
