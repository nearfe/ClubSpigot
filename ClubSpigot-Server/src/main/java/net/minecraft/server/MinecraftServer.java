...
import club.minemen.spigot.random.FastRandom;
...
import club.minemen.spigot.tickloop.ReentrantIAsyncHandler;
import club.minemen.spigot.tickloop.TasksPerTick;
import club.minemen.spigot.world.WorldTickManager;
// ClubSpigot end
...
public abstract class MinecraftServer extends ReentrantIAsyncHandler<TasksPerTick> implements ICommandListener, IAsyncTaskHandler, IMojangStatistics {
	...
	public java.util.Queue<Runnable> priorityProcessQueue = new java.util.concurrent.ConcurrentLinkedQueue<Runnable>(); // ClubSpigot
	public int autosavePeriod;
	// CraftBukkit end

	// ClubSpigot - instance
	protected ClubSpigot clubSpigot;
	
	// ClubSpigot - MSPT for tps command
	private double lastMspt;

	// ClubSpigot start - backport modern tick loop
	...
	public MinecraftServer(OptionSet options, Proxy proxy, File file1, Thread thread) {
		super("Server"); // ClubSpigot - backport modern tick loop
	...
	public void run() {
		...
		try {
			...
			if (this.init()) {
				//ClubSpigot - statistics
				disableStatistics = true;
				// ClubSpigot start - implement thread affinity
				if (ClubSpigotConfig.threadAffinity) {
					LOGGER.info(" ");
					...
					ClubSpigot.debug(AffinityLock.dumpLocks());

				}
				// ClubSpigot end

				// ClubSpigot - parallel worlds
				...
				while (this.isRunning) {
                    ...
                    if (i > 5000L && this.nextTickTime - this.lastOverloadWarning >= 30000L && ticks > 500) { // CraftBukkit // ClubSpigot - prevent display of overload on first 500 ticks
                        long j = i / 50L;
			...
			// ClubSpigot start - thread affinity
			if (lock != null) {
				lock.release();
				MinecraftServer.LOGGER.info("Released CPU " + lock.cpuId() + " from server usage.");
			}
			// ClubSpigot end
			// ClubSpigot start - stop statistics connection
			Thread statisticsThread = null;
			if (disableStatistics) {
				StatisticsClient client = this.getClubSpigot().getClient();
			...
			// ClubSpigot - wait for statistics to finish stopping
			try {
				if (this.getClubSpigot().getClient().isConnected) {
...
//                }
	...
	// ClubSpigot - backport modern tick loop
...
//        this.methodProfiler.b();
	...
	public void B() {
		...
		// ClubSpigot - move to WorldTickManager
...
//		SpigotTimings.bukkitSchedulerTimer.stopTiming(); // Spigot
		...
		// ClubSpigot start - priority process queue
	...
	public String getServerModName() {
		return ClubSpigotConfig.serverBrandName; // [Nacho-0035] // NachoSpigot - NachoSpigot > // TacoSpigot - TacoSpigot //
	...
	// ClubSpigot start - backport modern tick loo
	...
	// ClubSpigot - instance
	public ClubSpigot getClubSpigot() {
		return this.clubSpigot;
	}
	
	// ClubSpigot - MSPT (milliseconds per tick)
