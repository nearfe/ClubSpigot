...
import club.minemen.spigot.commons.IPUtils;
...
public class DedicatedServer extends MinecraftServer implements IMinecraftServer {
    ...
    public DedicatedServer(joptsimple.OptionSet options, Thread thread1) { // ClubSpigot - backport modern tick loop
        super(options, Proxy.NO_PROXY, DedicatedServer.a, thread1);
		// CraftBukkit end
		if (!ClubSpigotConfig.disableInfiniSleeperThreadUsage) {
	...
	protected boolean init() throws IOException {
		...
		if (!this.p.a() && !eulaAgreed) { // Spigot
			...
			ClubSpigotConfig.init((File) options.valueOf("clubspigot-settings"));
			// ClubSpigot end
			...
			if (!this.getOnlineMode()) {
				...
				if (org.spigotmc.SpigotConfig.bungee) {
					...
					if (!ClubSpigotConfig.stopNotifyBungee) {
						DedicatedServer.LOGGER
								.warn("---------------------------- ClubSpigot Checker ----------------------------");
						DedicatedServer.LOGGER.warn(
								"If you don't want to see this message anymore, set \"settings.stop-notify-bungee\" to \"true\" in \"clubspigot.yml\"!");
			...
			if (!NameReferencingFileConverter.a(this.propertyManager)) {
				...
				this.clubSpigot = new ClubSpigot(); // ClubSpigot
